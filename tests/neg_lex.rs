//! Negative lexer tests that should fail in the test CPU oracle (lex_on_test_cpu returns Err).

use laniusc::lexer::test_cpu::lex_on_test_cpu;

#[test]
fn unterminated_string_eof() {
    let src = "s=\"hello"; // missing closing quote
    assert!(
        lex_on_test_cpu(src).is_err(),
...
fn newline_in_string() {
    ...
    assert!(
        lex_on_test_cpu(src).is_err(),
        "newline in string should error"
    );
}
...
fn unterminated_char_eof() {
    ...
    assert!(
        lex_on_test_cpu(src).is_err(),
        "unterminated char should error"
    );
}
