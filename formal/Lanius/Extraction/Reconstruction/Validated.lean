...
def linkFrom (grammar : Grammar) (view : ParseArtifactView artifact)
  ...
  | id, value :: rest, stack => do
    ...
    if ParsePostorder.nodeWithLookup grammar view lookup id value entries then
      linkFrom grammar view lookup (id + 1) rest (.node id value children :: stack)
    else none
...
theorem linkFrom_linked (grammar : Grammar) (view : ParseArtifactView artifact)
    ...
    (accepted : linkFrom grammar view lookup id nodes stack = some forest) :
  ...
  | cons value rest ih =>
    ...
    | some result =>
      ...
      change (if ParsePostorder.nodeWithLookup grammar view lookup id value
          (children.filterMap fun child => child.map fun tree => (tree.id, tree.value))
        then linkFrom grammar view lookup (id + 1) rest (.node id value children :: tail)
...
theorem linkFrom_nodes (grammar : Grammar) (view : ParseArtifactView artifact)
    ...
    (accepted : linkFrom grammar view grammar.production? id remaining stack = some forest) :
  ...
  | cons value rest ih =>
    ...
    | some result =>
      ...
      change (if ParsePostorder.node grammar view id value
          (children.filterMap fun child => child.map fun tree => (tree.id, tree.value))
        then linkFrom grammar view grammar.production? (id + 1) rest (.node id value children :: tail)
        else none) = some forest at accepted
      split at accepted
...
def checkedForest (grammar : Grammar) (view : ParseArtifactView artifact)
  ...
  match accepted : linkFrom grammar view lookup 0 artifact.parse_nodes [] with
  | none => none
  | some forest => some (forest.attach.map fun tree =>
    ⟨tree.val, ParseTree.link_sound artifact.parse_nodes forest
      (linkFrom_linked grammar view lookup 0 artifact.parse_nodes [] forest accepted)
      tree.val tree.property⟩)

def checkedView (grammar : Grammar) (view : ParseArtifactView artifact)
    (lookup : Nat → Option Production) : Option SurfaceFile :=
  letI := ArtifactAccess.ofView view.artifactView
  do
    let forest ← checkedForest grammar view lookup
...
theorem checkedView_sound (grammar : Grammar) (view : ParseArtifactView artifact)
    (lookup : Nat → Option Production) (lookup_eq : lookup = grammar.production?)
    (surface : SurfaceFile) (accepted : checkedView grammar view lookup = some surface) :
  ...
  subst lookup
  cases found : linkFrom grammar view grammar.production? 0 artifact.parse_nodes [] with
  | none =>
    have absent : checkedForest grammar view grammar.production? = none := by
      unfold checkedForest
      split <;> simp_all
    simp [checkedView, absent] at accepted
  | some forest =>
    have same : checkedForest grammar view grammar.production? = ParseTree.checkedForest artifact.parse_nodes := by
      have original : ParseTree.link artifact.parse_nodes = some forest :=
        linkFrom_linked grammar view grammar.production? 0 artifact.parse_nodes [] forest found
