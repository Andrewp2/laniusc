...
theorem body_evaluates (leading trailing : List Int)
    ...
    Lanius.FunctionalView.Stateful.Acyclic.run? TM SM
      (Model.keywordWorld (leading ++ [first, second, third, fourth] ++ trailing))
      (KeywordSemantics.lengthEnvironment leading [first, second, third, fourth] trailing)
    ...
    some (.returned (some (.signed .i32
      ...
      KeywordSemantics.lengthEnvironment leading
        [first, second, third, fourth] trailing) := by
  ...
  cases selected : KeywordLengthSemantics.firstMatchingConstant
      (KeywordSemantics.loaded4Environment leading spelling trailing
        first second third fourth) KeywordCommand.length4Rules with
  | none =>
      have loadResult := KeywordSemantics.directLoad4_noMatch_evaluates leading spelling
        trailing first second third fourth rfl bounded selected
      have branch4 := KeywordDispatchSemantics.lengthBranch_true leading spelling trailing 4
      ...
      have decision := KeywordSemantics.length4_decisionValue leading trailing
      ...
      have identifierResult : Model.keywordKind spelling 0 4 =
  ...
  | some constant =>
      ...
      | some declaration =>
          ...
          have valueEq : declaration.value = .signed .i32
              (Model.keywordKind spelling 0 4) := by
            simpa [spelling] using Option.some.inj decision
          have loadResult := KeywordSemantics.directLoad4_match_evaluates leading spelling
            trailing first second third fourth constant declaration rfl bounded selected found
          have branch4 := KeywordDispatchSemantics.lengthBranch_true leading spelling trailing 4
            (KeywordCommand.directLoad4
              (KeywordCommand.directChoices KeywordCommand.length4Rules))
            (.returned (some declaration.value)) (by simp [spelling]) loadResult
          have tail4 := KeywordDispatchSemantics.sequence_stop world environment
            (KeywordCommand.directLengthBranch 4
              (KeywordCommand.directLoad4
                (KeywordCommand.directChoices KeywordCommand.length4Rules)))
            ...
            (some declaration.value) branch4
          have tail3 := KeywordDispatchSemantics.sequence_next world environment _ _ _ branch3 tail4
          have all := KeywordDispatchSemantics.sequence_next world environment _ _ _ branch2 tail3
          simpa [KeywordDispatchSemantics.body, spelling, world, environment, valueEq] using all

theorem command_evaluates (leading trailing : List Int)
    (first second third fourth : Int)
    (bounded : (leading ++ [first, second, third, fourth] ++ trailing).length ≤
      2147483647) :
    Lanius.FunctionalView.Stateful.Acyclic.run? TM SM
      (Model.keywordWorld (leading ++ [first, second, third, fourth] ++ trailing))
      (Model.keywordEnvironment (leading ++ [first, second, third, fourth] ++ trailing)
  ...
  let spelling := [first, second, third, fourth]
  have body := body_evaluates leading trailing first second third fourth bounded
