...
/-- The complete successful source iteration, including its counter increment
and both field-scope closures. Counter ownership is retained across the calls. -/
theorem Entry.success {result : Int} {contents : List Int}
    (entry : Entry before) (word : Word.Checked program byte digit)
    (indexOwned : (Assertion.localPointsTo 7 indexCell (some (.signed .i32 entry.index))).holds before)
    ...
    ∃ after, Executes program.core before (step word.source.function.id) .next after ∧
      ...
      (Assertion.localPointsTo 6 entry.cursorCell (some (.signed .i32 result))).holds after ∧
      (Assertion.localPointsTo 7 indexCell (some (.signed .i32 (entry.index + 1 : Nat)))).holds after ∧
  ...
  have indexFirst := bindLocal_preserves_localPointsTo_of_ne before 8 7
    (.signed .i32 entry.assignment.first) indexCell _ entry.wellFormed (by decide) indexOwned
  have indexScope := bindLocal_preserves_localPointsTo_of_ne firstState 9 7
    (.signed .i32 (secondWord entry.assignment)) indexCell _ firstWF (by decide) indexFirst
  have scopeWF : StateWellFormed entry.scope := bindLocal_preserves_well_formed _ _ _ firstWF
  ...
  obtain ⟨advanced, incremented, advancedWF, indexAdvanced, modifies⟩ := evaluatesIncrementOwnedI32Local
    program.core written 7 indexCell entry.index effect.wellFormed indexWritten
  ...
  have closed := CellEffect.closeLocal before 8 (.signed .i32 entry.assignment.first) entry.wellFormed
    (CellEffect.closeLocal firstState 9 (.signed .i32 (secondWord entry.assignment)) firstWF combined)
