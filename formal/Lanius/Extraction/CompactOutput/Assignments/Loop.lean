import Lanius.Extraction.CompactOutput.Assignments.State
...
def encodeAll (assignments : List Assignment) : List Nat :=
  assignments.flatMap (fun assignment => encoding assignment.first (secondWord assignment))

def Completed (memory : Memory) (outcome : AppendOutcome) (after : State) : Prop :=
  match outcome with
  | .done position contents => Owned memory memory.assignments.length position contents after
  | .full _ => True
...
theorem execute_loop (word : Word.Checked program byte digit) (remaining : List Assignment)
    (owned : Owned memory index position contents before)
    (suffix : memory.assignments.drop index = remaining) :
  ...
  | nil =>
    have finished : index = memory.assignments.length := by
  ...
  | cons assignment rest ih =>
    have bound : index < memory.assignments.length := by
      have length := congrArg List.length suffix
      simp only [List.length_drop, List.length_cons] at length
      omega
    have selected : memory.assignments[index] = assignment ∧ memory.assignments.drop (index + 1) = rest := by
      rw [List.drop_eq_getElem_cons bound] at suffix
      exact List.cons.inj suffix
    let entry := owned.entry bound
    have chosen : entry.assignment = assignment := selected.1
    have conditionRun : Evaluates program.core before condition (.boolean true) before := by
      simpa only [ne_eq, show index ≠ memory.assignments.length by omega, not_false_eq_true, decide_true] using owned.condition program.core
    have encoded : appendAll memory.capacity (encodeAll (assignment :: rest)) position contents =
      ...
      change appendAll memory.capacity
        (encoding assignment.first (secondWord assignment) ++ encodeAll rest) position contents =
        (appendAll memory.capacity (encoding entry.assignment.first (secondWord entry.assignment))
    ...
    | done nextCursor updated =>
      ...
      have size : updated.length = contents.length := by
        have length := appendAll_length memory.capacity
          (encoding entry.assignment.first (secondWord entry.assignment)) position contents
