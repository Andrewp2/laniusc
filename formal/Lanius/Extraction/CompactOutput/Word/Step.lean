...
theorem append (byte : CheckedByte program) (digit : CheckedDigit program)
  ...
  obtain ⟨written, call, backing, writeEffect, writeHeap⟩ := append_digit byte digit position memory.capacity
  ...
  obtain ⟨after, assigned, cursor, combined, assignmentEffect, assignmentHeap, _⟩ := evaluatesOwnedLocalSet owned.cursor call writeEffect cursorStill
  ...
  exact ⟨after, assigned, owned.transition effect appended_length outputAfter cursor shiftAfter, effect,
    writeHeap.trans assignmentHeap⟩
