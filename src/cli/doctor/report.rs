    ...
    let document = serde_json::json!({
        ...
        "toolchain": {
            ...
            "shader_artifacts": {
                ...
                "count": shader_artifact_u64_metadata(option_env!("LANIUS_SHADER_ARTIFACT_COUNT")),
                "largest": {
                    "status": known_or_unknown_status(option_env!("LANIUS_SHADER_ARTIFACT_MAX_BYTES")),
                    "name": option_env!("LANIUS_SHADER_ARTIFACT_MAX_NAME").unwrap_or("unknown"),
                    "bytes": shader_artifact_u64_metadata(option_env!("LANIUS_SHADER_ARTIFACT_MAX_BYTES")),
                },
                "size_guard": {
                    "status": option_env!("LANIUS_SHADER_SIZE_GUARD_STATUS").unwrap_or("unknown"),
                    "max_spv_bytes": shader_artifact_guard_max_metadata(
                        option_env!("LANIUS_SHADER_SIZE_GUARD_MAX_BYTES")
                    ),
...
fn shader_artifact_u64_metadata(value: Option<&str>) -> serde_json::Value {
    value
        .and_then(|value| value.parse::<u64>().ok())
        .map_or_else(|| serde_json::json!("unknown"), serde_json::Value::from)
}

fn shader_artifact_guard_max_metadata(value: Option<&str>) -> serde_json::Value {
    match value {
