...
impl HirItemFieldsPass {
    pub fn new(device: &wgpu::Device) -> Result<Self> {
        let data = crate::gpu::passes_core::make_pass_data(
            device,
            "hir_item_fields",
            "main",
            include_bytes!(concat!(env!("OUT_DIR"), "/shaders/hir_item_fields.spv")),
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/hir_item_fields.reflect.json"
            )),
        )?;
        Ok(Self { data })
    }
...
impl Pass<ParserBuffers, crate::parser::gpu::debug::DebugOutput> for HirItemFieldsPass {
