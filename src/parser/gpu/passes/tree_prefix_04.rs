...
use crate::{
    ...
    parser::gpu::buffers::{ParserBuffers, TreePrefixMaxBuildStep},
};
    ...
    pub fn new(device: &wgpu::Device) -> Result<Self> {
        let data = crate::gpu::passes_core::make_pass_data(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/tree_prefix_04_build_max_tree.spv"
            )),
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/tree_prefix_04_build_max_tree.reflect.json"
            )),
        )?;
        Ok(Self { data })
    }
...
impl Pass<ParserBuffers, crate::parser::gpu::debug::DebugOutput> for TreePrefixMaxBuildPass {
