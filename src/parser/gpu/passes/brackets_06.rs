use std::collections::HashMap;

use anyhow::Result;
use encase::ShaderType;

use crate::{
    gpu::passes_core::{DispatchDim, Pass, PassData},
    parser::gpu::buffers::ParserBuffers,
};

#[repr(C)]
#[derive(Clone, Copy, ShaderType)]
pub struct Params {
    pub n_sc: u32,
    pub n_layers: u32,
}

pub struct BracketsScatterByLayerPass {
    data: PassData,
}

impl BracketsScatterByLayerPass {
    pub fn new(device: &wgpu::Device) -> Result<Self> {
        let data = crate::gpu::passes_core::make_pass_data(
            device,
            "brackets_06_scatter_by_layer",
            "main",
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/brackets_06_scatter_by_layer.spv"
            )),
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/brackets_06_scatter_by_layer.reflect.json"
            )),
        )?;
        Ok(Self { data })
    }
}

impl Pass<ParserBuffers, crate::parser::gpu::debug::DebugOutput> for BracketsScatterByLayerPass {
    const NAME: &'static str = "brackets_06_scatter_by_layer";
    const DIM: DispatchDim = DispatchDim::D1;

    fn from_data(data: PassData) -> Self {
        Self { data }
    }
    fn data(&self) -> &PassData {
        &self.data
    }

    fn create_resource_map<'a>(
        &self,
        b: &'a ParserBuffers,
    ) -> HashMap<String, wgpu::BindingResource<'a>> {
        HashMap::from([
            ("gParams".into(), b.b06_params.as_entire_binding()),
            ("sc_stream".into(), b.out_sc.as_entire_binding()),
            ("layer".into(), b.b_layer.as_entire_binding()),
            ("off_push".into(), b.b_off_push.as_entire_binding()),
            ("off_pop".into(), b.b_off_pop.as_entire_binding()),
            ("cur_push".into(), b.b_cur_push.as_entire_binding()),
            ("cur_pop".into(), b.b_cur_pop.as_entire_binding()),
            (
                "pushes_by_layer".into(),
                b.b_pushes_by_layer.as_entire_binding(),
            ),
            (
                "pops_by_layer".into(),
                b.b_pops_by_layer.as_entire_binding(),
            ),
        ])
    }
}
