    ...
    pub fn new(device: &wgpu::Device) -> Result<Self> {
        let data = crate::gpu::passes_core::make_pass_data(
            ...
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/brackets_pse_04_pair_by_layer.spv"
            )),
            include_bytes!(concat!(
                env!("OUT_DIR"),
                "/shaders/brackets_pse_04_pair_by_layer.reflect.json"
            )),
        )?;
        Ok(Self { data })
    }
...
impl Pass<ParserBuffers, crate::parser::gpu::debug::DebugOutput> for BracketsPsePairPass {
