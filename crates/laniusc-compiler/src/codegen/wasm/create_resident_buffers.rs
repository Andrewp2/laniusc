...
impl GpuWasmCodeGenerator {
    pub(super) fn create_resident_buffers(
        ...
        let expr_order = self.create_wasm_expr_order(
            ...
            inputs.expressions.parent_node,
            None,
            inputs,
            &working_buffers,
            false,
        )?;
        let compact_hir_capacity = u32::try_from(inputs.canonical_hir.expr_root.size() / 4)
            .map_err(|_| anyhow::anyhow!("compact HIR capacity exceeds u32"))?
            .max(1);
        let compact_expr_order = self.create_wasm_expr_order(
            device,
            compact_hir_capacity,
            inputs.canonical_hir.expr_root,
            inputs.canonical_hir.expr_parent,
            Some(inputs.canonical_hir.core),
            inputs,
            &working_buffers,
            true,
        )?;
        ...
        } = self.create_wasm_body_plan_bind_groups(
            device,
            inputs,
            &working_buffers,
            &expr_order,
        ...
        Ok(ResidentWasmBuffers {
            ...
            _body_let_init_hir_by_decl_token_buf: body_let_init_hir_by_decl_token_buf,
            ...
            _compact_expr_subtree_total_buf: compact_expr_subtree_total_buf,
            _compact_expr_subtree_features_buf: compact_expr_subtree_features_buf,
