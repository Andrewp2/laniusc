...
pub struct CompilationUnitLimits {
    pub max_source_bytes: usize,
    pub max_source_files: usize,
}

impl Default for CompilationUnitLimits {
    fn default() -> Self {
        Self {
...
/// Bounded compiler unit produced from contiguous source files.
...
#[derive(Clone, Debug, Default, PartialEq, Eq)]
/// Collected bounded compiler units for a source pack.
pub struct CompilationUnitPlan {
    pub units: Vec<CompilationUnit>,
}

impl CompilationUnitPlan {
    /// Builds bounded units for a single-library in-memory source pack.
    pub fn from_source_pack<S: AsRef<str>>(sources: &[S], limits: CompilationUnitLimits) -> Self {
        ...
        .unwrap_or_else(|()| unreachable!("infallible compilation-unit collection failed"));
        Self { units }
    }

    /// Builds bounded units from sources and explicit library ids.
    ...
    /// Builds bounded units from precomputed source-file facts.
    ...
    /// Streams units to a visitor without retaining the full plan.
    ...
    where
        I: IntoIterator<Item = SourceFileUnitInput>,
        F: FnMut(CompilationUnit) -> Result<(), E>,
    {
        Self::try_for_each_from_fallible_files(files.into_iter().map(Ok), limits, visit)
    }

    /// Streams units from a fallible source-file iterator.
    ...
    /// Returns the number of units in this plan.
    ...
    /// Returns the largest source-byte total among planned units.
    ...
    /// Returns the largest source-file count among planned units.
...
impl UnitBuilder {
    ...
    /// Emits the pending compiler unit and resets the builder, or returns `None` when empty.
