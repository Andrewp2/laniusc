...
fn main() {
    ...
    if let Err(err) = pollster::block_on(laniusc_compiler::lexer::lex_on_gpu("warmup")) {
...
async fn run_once(
    ...
    let t1 = Instant::now();
    let gpu = laniusc_compiler::lexer::lex_on_gpu(src)
        .await
...
fn compare_streams(
    ...
    gpu: &[laniusc_compiler::lexer::Token],
) -> bool {
