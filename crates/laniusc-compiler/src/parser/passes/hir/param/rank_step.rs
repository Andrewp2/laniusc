...
crate::gpu::passes_core::impl_static_shader_pass!(
    HirParamRankStepPass,
    label: "hir_param_rank_step",
    shader: "parser/hir/param/rank_step"
);
...
impl HirParamRankStepPass {
    ...
    fn record_steps_inner(
        ...
        if steps % 2 == 1 {
            ...
            for (src, dst) in [
                ...
                (&buffers.hir_param_link_b, &buffers.hir_param_link_a),
                (&buffers.hir_param_rank_b, &buffers.hir_param_rank_a),
            ] {
    ...
    fn record_step(
        ...
        let (owner_in, link_in, rank_in, owner_out, link_out, rank_out) = if read_from_a {
            (
                &buffers.hir_param_owner_a,
                &buffers.hir_param_link_a,
                &buffers.hir_param_rank_a,
                &buffers.hir_param_owner_b,
                &buffers.hir_param_link_b,
                &buffers.hir_param_rank_b,
            )
        } else {
            (
                &buffers.hir_param_owner_b,
        ...
        let resources: HashMap<String, wgpu::BindingResource<'_>> = HashMap::from([
            (
                "gHirParam".into(),
                buffers.hir_param_fields_params.as_entire_binding(),
            ),
            (
            ...
            ("hir_param_owner_out".into(), owner_out.as_entire_binding()),
            ("hir_param_link_out".into(), link_out.as_entire_binding()),
            ("hir_param_rank_out".into(), rank_out.as_entire_binding()),
        ]);

        let bind_group = bind_group::create_bind_group_from_reflection(
            device,
            Some("hir_param_rank_step"),
