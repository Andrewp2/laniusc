...
/// Pointer-jump pass that propagates type-argument owners and ranks through type syntax.
pub struct HirTypeArgRankStepPass {
    data: PassData,
}

crate::gpu::passes_core::impl_static_shader_pass!(
    HirTypeArgRankStepPass,
    label: "hir_type_arg_rank_step",
    shader: "parser/hir/type/arg/rank_step"
);

impl HirTypeArgRankStepPass {
    /// Records all type-argument rank propagation steps with direct dispatch sizing.
    pub fn record_steps(
        &self,
        device: &wgpu::Device,
    ...
    /// Records all type-argument rank propagation steps with indirect dispatch sizing.
    ...
    fn record_steps_inner(
        ...
        if steps % 2 == 1 {
            ...
            for (src, dst) in [
                (&buffers.hir_type_arg_owner_b, &buffers.hir_type_arg_owner_a),
    ...
    fn record_step(
        ...
        let (owner_in, link_in, rank_in, owner_out, link_out, rank_out) = if read_from_a {
            (
                &buffers.hir_type_arg_owner_a,
                &buffers.hir_type_arg_link_a,
                &buffers.hir_type_arg_rank_a,
        ...
        let resources: HashMap<String, wgpu::BindingResource<'_>> = HashMap::from([
            ...
            (
                "hir_type_arg_owner_out".into(),
                owner_out.as_entire_binding(),
            ),
            ("hir_type_arg_link_out".into(), link_out.as_entire_binding()),
            ("hir_type_arg_rank_out".into(), rank_out.as_entire_binding()),
        ]);

        let bind_group = bind_group::create_bind_group_from_reflection(
            device,
            Some("hir_type_arg_rank_step"),
        ...
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("hir_type_arg_rank_step"),
