...
/// Pointer-jumps raw parse-tree parent links to compute exact node depths.
pub struct TreeDepthStepPass {
    data: PassData,
}

crate::gpu::passes_core::impl_static_shader_pass!(
    TreeDepthStepPass,
    label: "tree_depth_step",
    shader: "parser/tree/depth/step"
);

impl TreeDepthStepPass {
    ...
    fn record_step(
        ...
        let resources = HashMap::from([
            ("gTree".into(), buffers.tree_span_params.as_entire_binding()),
            (
            ...
            ("tree_depth_link_in".into(), link_in.as_entire_binding()),
            ("tree_depth_value_in".into(), value_in.as_entire_binding()),
            ("tree_depth_link_out".into(), link_out.as_entire_binding()),
            ("tree_depth_value_out".into(), value_out.as_entire_binding()),
        ]);
        let group = bind_group::create_bind_group_from_reflection(
            device,
            Some("tree_depth_step"),
