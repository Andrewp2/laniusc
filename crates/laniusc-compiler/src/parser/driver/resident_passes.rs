    ...
    pub(super) fn record_ll1_resident_passes(
        ...
        if include_tree {
            ...
            self.passes
                ...
                .record_pass_indirect(&mut ctx, &bufs.tree_active_dispatch_args)?;
            stamp_timer(timer_ref, ctx.encoder, "parser.tree_depth_block_max");
            self.passes.tree_depth_schedule.record_pass(
                &mut ctx,
                crate::gpu::passes_core::InputElements::Elements1D(256),
            )?;
            stamp_timer(timer_ref, ctx.encoder, "parser.tree_depth_schedule");
            ...
            if include_hir_spans {
                ...
                self.passes.hir_list_rank_step.record(
                    ctx.device,
                    ctx.encoder,
                    ctx.buffers,
                    &bufs.hir_type_fields_params,
                ...
                self.passes
                    ...
                    .record_pass_indirect(&mut ctx, &bufs.tree_active_dispatch_args)?;
                self.passes.hir_type_root_owner_step.record_steps_indirect(
                    ctx.device,
                    ctx.encoder,
                    ctx.buffers,
                    &bufs.tree_active_dispatch_args,
                ...
                if parser_compute_pass_batching_enabled(timer_ref) {
                    ...
                    batch.record_pass_indirect_cached(
                        ...
                        ctx.buffers,
                        bg_cache,
                        &self.passes.hir_postfix_fields,
                        &bufs.hir_semantic_dispatch_args,
                    )?;
                    batch.record_pass_indirect_cached(
                        ctx.device,
                    ...
                    self.passes
                        .hir_postfix_fields
                        .record_pass_indirect(&mut ctx, &bufs.hir_semantic_dispatch_args)?;
                    stamp_timer(timer_ref, ctx.encoder, "parser.hir_postfix_fields");
                    self.passes
                        .hir_member_spans
                        .record_pass_indirect(&mut ctx, &bufs.hir_semantic_dispatch_args)?;
                ...
                let raw_expression_records = crate::parser::passes::record_raw_expression_spans(
                    &mut ctx,
                    &self.passes,
                    &bufs.hir_semantic_dispatch_args,
                    &bufs.tree_active_dispatch_args,
                )?;
                stamp_timer(timer_ref, ctx.encoder, "parser.hir_raw_expression_spans");
                self.passes
                ...
                // The compact statement record is now durable, so its raw
                ...
                self.passes.hir_canonical_call_arg_mark.record_pass(
                    &mut ctx,
                ...
                crate::parser::passes::record_canonical_call_arguments(&mut ctx, &self.passes)?;
                stamp_timer(timer_ref, ctx.encoder, "parser.hir_call_arg_compact");
                self.passes
                ...
                crate::parser::passes::record_canonical_array_elements(&mut ctx, &self.passes)?;
                stamp_timer(timer_ref, ctx.encoder, "parser.hir_array_element_compact");
                if bufs.parser_feature_flags & crate::lexer::features::PARSER_FEATURE_MATCHES != 0 {
                    self.passes
                        .hir_match_arm_owner_init
                        .record_pass_indirect(&mut ctx, &bufs.tree_active_dispatch_args)?;
                    stamp_timer(timer_ref, ctx.encoder, "parser.hir_match_arm_owner_init");
                    self.passes.hir_tree_relations.record_steps_for_buffers(
                        ctx.device,
                        ctx.encoder,
                        ctx.buffers,
                ...
                crate::parser::passes::record_canonical_fields(&mut ctx, &self.passes)?;
                stamp_timer(timer_ref, ctx.encoder, "parser.hir_field_compact");
                // Context pointer jumping aliases the two raw struct-list
                // workspaces. Start it only after both declaration and
                // literal fields have crossed the compact HIR boundary.
                self.passes
                ...
                crate::parser::passes::record_canonical_hir_materialization(
                    &mut ctx,
                    &self.passes,
                    timer_ref,
                )?;
                stamp_timer(
                    timer_ref,
                    ctx.encoder,
                    "parser.hir_canonical_materialization",
                );
            }
        }
        Ok(())
