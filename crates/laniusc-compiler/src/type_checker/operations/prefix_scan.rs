use super::super::*;

#[derive(Clone, Copy)]
pub(in crate::type_checker) struct PrefixScanPasses<'a> {
    pub local: &'a PassData,
    pub hierarchy_up: &'a PassData,
    pub hierarchy_down: &'a PassData,
    pub apply: &'a PassData,
}

impl<'a> From<&'a TypeCheckPasses> for PrefixScanPasses<'a> {
    fn from(passes: &'a TypeCheckPasses) -> Self {
        Self {
            local: &passes.counted_scan_local,
            hierarchy_up: &passes.counted_scan_hierarchy_up,
            hierarchy_down: &passes.counted_scan_hierarchy_down,
            apply: &passes.counted_scan_apply,
        }
    }
}

pub(in crate::type_checker) type PrefixScanBuffers<'a> =
    crate::gpu::compiler_graph::PrefixScanResources<&'a wgpu::Buffer>;

struct HierarchyStep {
    _params: LaniusBuffer<CountedScanHierarchyParams>,
    group: wgpu::BindGroup,
    work_items: u32,
}

pub(in crate::type_checker) struct PrefixScanOperation {
    label: &'static str,
    passes: [PassData; 4],
    dispatch_args: wgpu::Buffer,
    _params: LaniusBuffer<NameScanParams>,
    local: wgpu::BindGroup,
    up: Vec<HierarchyStep>,
    down: Vec<HierarchyStep>,
    apply: wgpu::BindGroup,
}

impl PrefixScanOperation {
    pub(in crate::type_checker) fn from_pair_spec(
        device: &wgpu::Device,
        passes: PrefixScanPasses<'_>,
        resources: &ResourceMap<'_>,
        spec: crate::gpu::compiler_graph::PrefixScanPairSpec,
    ) -> Result<(Self, Self)> {
        Ok((
            Self::from_resource_names(device, spec.left_label, passes, resources, spec.left)?,
            Self::from_resource_names(device, spec.right_label, passes, resources, spec.right)?,
        ))
    }

    pub(in crate::type_checker) fn from_spec(
        device: &wgpu::Device,
        passes: PrefixScanPasses<'_>,
        resources: &ResourceMap<'_>,
        spec: crate::gpu::compiler_graph::PrefixScanSpec,
    ) -> Result<Self> {
        Self::from_resource_names(device, spec.passes.local, passes, resources, spec.resources)
    }

    pub(in crate::type_checker) fn with_workspace(
        device: &wgpu::Device,
        label: &'static str,
        params: NameScanParams,
        passes: PrefixScanPasses<'_>,
        count: &wgpu::Buffer,
        dispatch_args: &wgpu::Buffer,
        input: &wgpu::Buffer,
        output_prefix: &wgpu::Buffer,
        total: &wgpu::Buffer,
        workspace: PrefixScanWorkspace<&LaniusBuffer<u32>>,
    ) -> Result<Self> {
        Self::new(
            device,
            label,
            params,
            passes,
            PrefixScanBuffers {
                count,
                dispatch_args,
                input,
                output_prefix,
                total,
                local_prefix: workspace.local_prefix,
                block_sum: workspace.block_sum,
                block_prefix: workspace.block_prefix,
                hierarchy: workspace.hierarchy,
            },
        )
    }

    pub(in crate::type_checker) fn from_resource_names(
        device: &wgpu::Device,
        label: &'static str,
        passes: PrefixScanPasses<'_>,
        resources: &ResourceMap<'_>,
        names: crate::gpu::compiler_graph::PrefixScanResources<&str>,
    ) -> Result<Self> {
        let buffer = |name| buffer_from_resources(resources, name);
        let n_items = resources.logical_u32_count(names.input)?;
        Self::new(
            device,
            label,
            NameScanParams {
                n_items,
                n_blocks: n_items.div_ceil(256).max(1),
                scan_step: 0,
            },
            passes,
            PrefixScanBuffers {
                count: buffer(names.count)?,
                input: buffer(names.input)?,
                output_prefix: buffer(names.output_prefix)?,
                total: buffer(names.total)?,
                dispatch_args: buffer(names.dispatch_args)?,
                local_prefix: buffer(names.local_prefix)?,
                block_sum: buffer(names.block_sum)?,
                block_prefix: buffer(names.block_prefix)?,
                hierarchy: buffer(names.hierarchy)?,
            },
        )
    }

    pub(in crate::type_checker) fn new(
        device: &wgpu::Device,
        label: &'static str,
        params: NameScanParams,
        passes: PrefixScanPasses<'_>,
        buffers: PrefixScanBuffers<'_>,
    ) -> Result<Self> {
        let levels = crate::gpu::scan::hierarchical_scan_levels(params.n_blocks);
        let params_buffer = uniform_from_val(device, &format!("{label}.params"), &params);
        let bind =
            |suffix: &str, pass: &PassData, bindings: &[(&str, wgpu::BindingResource<'_>)]| {
                bind_group::create_bind_group_from_bindings(
                    device,
                    Some(&format!("{label}.{suffix}")),
                    pass,
                    0,
                    bindings,
                )
            };
        let local = bind(
            "local",
            passes.local,
            &[
                ("gScan", params_buffer.as_entire_binding()),
                ("scan_count", buffers.count.as_entire_binding()),
                ("scan_input", buffers.input.as_entire_binding()),
                (
                    "scan_local_prefix",
                    buffers.local_prefix.as_entire_binding(),
                ),
                ("scan_block_sum", buffers.block_sum.as_entire_binding()),
            ],
        )?;
        let hierarchy = |suffix,
                         pass,
                         index,
                         level: crate::gpu::scan::HierarchicalScanLevel,
                         parent: Option<crate::gpu::scan::HierarchicalScanLevel>|
         -> Result<HierarchyStep> {
            let level_params = uniform_from_val(
                device,
                &format!("{label}.{suffix}.{index}.params"),
                &CountedScanHierarchyParams {
                    n_items: params.n_items,
                    n_blocks: params.n_blocks,
                    level_divisor: level.divisor,
                    level_offset: level.offset,
                    parent_divisor: parent.map_or(0, |value| value.divisor),
                    parent_offset: parent.map_or(0, |value| value.offset),
                },
            );
            let mut bindings = vec![
                ("gHierarchy", level_params.as_entire_binding()),
                ("scan_count", buffers.count.as_entire_binding()),
                (
                    "scan_block_prefix",
                    buffers.block_prefix.as_entire_binding(),
                ),
                ("scan_hierarchy", buffers.hierarchy.as_entire_binding()),
            ];
            if suffix == "up" {
                bindings.insert(2, ("scan_block_sum", buffers.block_sum.as_entire_binding()));
            }
            Ok(HierarchyStep {
                group: bind(suffix, pass, &bindings)?,
                _params: level_params,
                work_items: level.count,
            })
        };
        let up = levels
            .iter()
            .copied()
            .enumerate()
            .map(|(index, level)| {
                hierarchy(
                    "up",
                    passes.hierarchy_up,
                    index,
                    level,
                    levels.get(index + 1).copied(),
                )
            })
            .collect::<Result<Vec<_>>>()?;
        let down = (0..levels.len().saturating_sub(1))
            .rev()
            .map(|index| {
                hierarchy(
                    "down",
                    passes.hierarchy_down,
                    index,
                    levels[index],
                    Some(levels[index + 1]),
                )
            })
            .collect::<Result<Vec<_>>>()?;
        let apply = bind(
            "apply",
            passes.apply,
            &[
                ("gScan", params_buffer.as_entire_binding()),
                ("scan_count", buffers.count.as_entire_binding()),
                (
                    "scan_local_prefix",
                    buffers.local_prefix.as_entire_binding(),
                ),
                (
                    "scan_block_prefix",
                    buffers.block_prefix.as_entire_binding(),
                ),
                (
                    "scan_output_prefix",
                    buffers.output_prefix.as_entire_binding(),
                ),
                ("scan_total", buffers.total.as_entire_binding()),
            ],
        )?;
        Ok(Self {
            label,
            passes: [
                passes.local.clone(),
                passes.hierarchy_up.clone(),
                passes.hierarchy_down.clone(),
                passes.apply.clone(),
            ],
            dispatch_args: buffers.dispatch_args.clone(),
            _params: params_buffer,
            local,
            up,
            down,
            apply,
        })
    }

    pub(in crate::type_checker) fn record(&self, encoder: &mut wgpu::CommandEncoder) -> Result<()> {
        record_compute_indirect(
            encoder,
            &self.passes[0],
            &self.local,
            self.label,
            &self.dispatch_args,
        )?;
        for step in &self.up {
            record_compute(
                encoder,
                &self.passes[1],
                &step.group,
                self.label,
                step.work_items,
            )?;
        }
        for step in &self.down {
            record_compute(
                encoder,
                &self.passes[2],
                &step.group,
                self.label,
                step.work_items,
            )?;
        }
        record_compute_indirect(
            encoder,
            &self.passes[3],
            &self.apply,
            self.label,
            &self.dispatch_args,
        )
    }

    pub(in crate::type_checker) fn record_pair(
        left: &Self,
        right: &Self,
        encoder: &mut wgpu::CommandEncoder,
    ) -> Result<()> {
        if left
            .passes
            .iter()
            .zip(&right.passes)
            .any(|(a, b)| a.shader_id != b.shader_id)
        {
            return Err(anyhow::anyhow!(
                "paired prefix scans must use the same scan kernels"
            ));
        }
        pair_indirect(
            encoder,
            &left.passes[0],
            &left.local,
            &left.dispatch_args,
            &right.local,
            &right.dispatch_args,
            "type_check.prefix_scan.pair.local",
        );
        for index in 0..left.up.len().max(right.up.len()) {
            pair_steps(
                encoder,
                &left.passes[1],
                left.up.get(index),
                right.up.get(index),
                "type_check.prefix_scan.pair.hierarchy_up",
            )?;
        }
        for index in 0..left.down.len().max(right.down.len()) {
            pair_steps(
                encoder,
                &left.passes[2],
                left.down.get(index),
                right.down.get(index),
                "type_check.prefix_scan.pair.hierarchy_down",
            )?;
        }
        pair_indirect(
            encoder,
            &left.passes[3],
            &left.apply,
            &left.dispatch_args,
            &right.apply,
            &right.dispatch_args,
            "type_check.prefix_scan.pair.apply",
        );
        Ok(())
    }
}

fn pair_indirect<'a>(
    encoder: &'a mut wgpu::CommandEncoder,
    pass: &'a PassData,
    left: &'a wgpu::BindGroup,
    left_args: &'a wgpu::Buffer,
    right: &'a wgpu::BindGroup,
    right_args: &'a wgpu::Buffer,
    label: &'static str,
) {
    count_recorded_compute_pass();
    let mut batch = crate::gpu::passes_core::ComputePassBatch::begin(encoder, label);
    batch.record_raw_indirect(pass, left, left_args);
    batch.record_raw_indirect(pass, right, right_args);
}

fn pair_steps<'a>(
    encoder: &'a mut wgpu::CommandEncoder,
    pass: &'a PassData,
    left: Option<&'a HierarchyStep>,
    right: Option<&'a HierarchyStep>,
    label: &'static str,
) -> Result<()> {
    count_recorded_compute_pass();
    let mut batch = crate::gpu::passes_core::ComputePassBatch::begin(encoder, label);
    if let Some(step) = left {
        batch.record_raw(pass, &step.group, step.work_items)?;
    }
    if let Some(step) = right {
        batch.record_raw(pass, &step.group, step.work_items)?;
    }
    Ok(())
}
