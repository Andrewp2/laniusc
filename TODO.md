...
- Replace the current witness-projected partial-parse table with a real LLP
  ...
  surface. The GPU now runs a full LL(1) acceptance/error pass and emits the
  exact LL(1) production stream from the generated grammar tables. The generated
  LLP pair-production stream is still a witness projection with conflicts, so
  it remains a parallel parser artifact rather than the correctness source for
  resident HIR/type/codegen.
...
- Add AST materialization from the production tree. The GPU currently returns a
  production-id tree. `src/hir.rs` now provides the semantic HIR shape from
  source tokens, so the remaining bridge is lowering the GPU production tree
  plus token spans into the same HIR.
